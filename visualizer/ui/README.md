...
| **Elasticsearch** | Yes — `source_url` (copied from S3 metadata) | `id`, `url` | `graph` = `urn:odis:{source}` |
| **Oxigraph data** (scribe) | via join to prov graph | RDF triples from body | `urn:odis:{source}` |
| **Oxigraph prov** (scribe) | Yes — `prov:hadPrimarySource` | `prov:wasDerivedFrom` `@id` when present | `urn:odis:prov:{source}` |

So: S3 ↔ ES join via `s3_key` / harvest URL; Oxigraph joins the same keys through the **prov** named graph (`prov:value` = s3 key, `prov:hadPrimarySource` = harvest URL) and content via entity `@id` or data graph `urn:odis:{source}`.
