...
def test_clear_graph_posts_update() -> None:
    ...
    clear_graph("http://localhost:7878", "urn:odis:medin", client=client)

    client.post.assert_called_once()
    args, kwargs = client.post.call_args
    assert args[0] == "http://localhost:7878/update"
    assert kwargs["headers"]["Content-Type"] == "application/sparql-update"
    assert b"CLEAR SILENT GRAPH <urn:odis:medin>" in kwargs["content"]
...
def test_load_nquads_posts_store() -> None:
    ...
    load_nquads("http://localhost:7878/", "<a> <b> <c> <urn:odis:x> .\n", client=client)
...
def test_replace_graphs_clears_both_then_loads() -> None:
    ...
    replace_graphs_with_nquads(
        "http://localhost:7878",
        ["urn:odis:medin", "urn:odis:prov:medin"],
        "<s> <p> <o> <urn:odis:medin> .\n",
        client=client,
    )
    ...
    assert any(b"urn:odis:medin" in c and b"CLEAR" in c for c in contents)
    assert any(b"urn:odis:prov:medin" in c for c in contents)
